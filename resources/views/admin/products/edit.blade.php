      ...
      <nav class="flex mb-1" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2">
          ...
          <li aria-current="page">
            <div class="flex items-center">
              <i data-feather="chevron-right" class="w-4 h-4 text-gray-400"></i>
              <span class="ml-1 text-sm text-gray-500 md:ml-2">Edit Produk</span>
            </div>
          </li>
        </ol>
      </nav>
      <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl">Edit Produk</h2>
    </div>
  </div>
    ...
    <form action="{{ route('admin.products.update', ['product' => $product->id]) }}" method="post" enctype="multipart/form-data">
      ...
      <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
        <div class="sm:col-span-2">
          ...
          <input type="text" name="name" id="name" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5" placeholder="Dodol Asli Kandangan" value="{{ $product->name }}" required="">
        </div>
        <div class="sm:col-span-2">
          <label for="seller" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Penjual <span class="text-red-500">*</span></label>
            <select id="seller" name="seller" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
              ...
              <option value="{{ $seller->id }}" @if($product->seller == $seller) selected @endif>
                {{ $seller->name }}
              </option>
              @endforeach
            </select>
        ...
        <div class="w-full">
          ...
          <div class="relative rounded-lg">
            ...
            <input type="number" name="price" id="price" class="block w-full rounded-lg border-0 py-2.5 pl-9 pr-3 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm" value="{{ str_replace('.', '', $product->price) }}">
          </div>
        </div>
        <div class="w-full">
        ...
        <div>
          ...
          <select id="category" name="category" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
            ...
            <option value="{{ $category->id }}" @foreach($product->categories as $c) @if($category->id == $c->id) selected @endif @endforeach>
              {{ $category->name }}
            </option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <div class="flex space-x-2.5">
            <img src="{{ $product->photo }}" alt="" class="border border-gray-300 w-11 h-11 rounded-lg object-cover">
        ...
        <div>
          ...
          <textarea id="description" name="description" rows="5" class="block p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{ $product->description }}</textarea>
        </div>
        <div>
          <label for="specification" class="block mb-2 text-sm font-medium text-gray-900">Spesifikasi</label>
          <textarea id="specification" name="specification" rows="5" class="block p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{ $product->specification }}</textarea>
        </div>
        <div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" name="archive" id="archive" @if($product->is_archived) value="1" @endif class="sr-only peer" @if($product->is_archived) checked @endif>
